...
def select_random_from_csv(file_path, row_count):
    ...
    return pandas.read_csv(file_path, header=None, skiprows=row, nrows=600)
...
try:
    ...
    sys.stdout.flush()
    producer.send("raw_stream", f"Send {message_count} messages from Washington Hospital.")
    producer.send("raw_stream", "info - Washington stream finished.")
