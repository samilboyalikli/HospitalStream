...
def blood_values_analysis(cbc):
    ...
    cbc = values_tied_on_gender(df=cbc,value_name="RBC",min_for_male=4700000,max_for_male=6100000,min_for_female=4200000,max_for_female=5400000)
    cbc = values_tied_on_gender(df=cbc,value_name="Hb",min_for_male=13.5,max_for_male=17.5,min_for_female=12.0,max_for_female=15.5)
    cbc = cbc.withColumn(f"status(Hct)"
        ,when((cbc.Gender=="MALE")&
            (regexp_replace(cbc.bloodValues["Hct"],"%","").cast(IntegerType())>=41)&
        ...
        .otherwise(
            when((cbc.Gender=="FEMALE")&
                ...
                (regexp_replace(cbc.bloodValues["Hct"],"%","").cast(IntegerType())<=44),True)
            .otherwise(False)))
